...
function generate-project {
    ...
    git add --all
    git branch -M main
    git commit -m "feat: generated sample project with python-package-course"
}
...
#    GITHUB_USERNAME - name of my github user, e.g. Carloslb17
...
EOF
    ...
    git commit -m 'feat!: populated from `python-package-course` template'

    # if GH_TOKEN is set, set the remote url to it
    if [[ -n "$GH_TOKEN" ]]; then
        git remote set-url origin "https://$GITHUB_USERNAME:$GH_TOKEN@github.com/Carloslb17/$REPO_NAME"
    fi
    git push origin "$UNIQUE_BRANCH_NAME"

    # open a PR from the feature branch into main
    gh pr create \
        --title "feat: populated from \`python-package-course\` template" \
        --body "This PR was generated by \`python-package-course\`" \
        --base main \
